        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSale in spuSaleAttrList" :key="spuSale.id">
                ...
                <dd
                  ...
                  v-for="spuSaleValue in spuSale.spuSaleAttrValueList"
                  :key="spuSaleValue.id"
                  @click="
                    changeActive(spuSaleValue, spuSale.spuSaleAttrValueList)
                  "
                >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 加入购物车，进入路由跳转之前，发请求把产品信息以请求的方式通知服务器，并存储 -->
...
export default {
  ...
  methods: {
    ...mapMutations(["GOODINFO"]),
    // 产品售卖属性值切换高亮
    changeActive(spuSaleValue, arr) {
      // 排他，把全部的属性变为0，只有点击的属性才为1
      arr.forEach((item) => {
        // console.log(item);
        item.isChecked = "0";
      });
    ...
    async addShopCart() {
     ...
     await this.$store.dispatch("addShopCart", {
        ...
        // 这种也可以
        // this.$router.push({name: 'addcartauccess'. query:{skuInfo:this.skuInfo,skuNum:this.count}})
        sessionStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo))
        this.$router.push(`/addcartauccess/? skuNum=${this.count}`)
      })
    },
  },
  created() {
